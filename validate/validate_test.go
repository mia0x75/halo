...
func init() {
	cfg := flag.String("c", "/Users/mxu/Public/Go/src/github.com/mia0x75/halo/bin/cfg.json", "configuration file")
	flag.Parse()
	g.ParseConfig(*cfg)
...
func TestGeneralRules(t *testing.T) {
	...
	for _, c := range cases {
		...
		if err != nil {
			t.Errorf("语法错误: %s, err: %s", c.Sql, err.Error())
			continue
		}
		s := &models.Statement{
			Content:    c.Sql,
...
func TestTableCreateColumnCountLimit(t *testing.T) {
	...
	for _, c := range Cases {
		...
		assert.Equal(valid, c.Valid, c.RuleName, c.Sql)
	}
}
