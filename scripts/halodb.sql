...
('CDB-L2-001','e4b6f058-f8e3-469d-b754-ac754c9bd07e',10,'新建数据库时允许的字符集',2,100,'in','[\"utf8mb4\",\"binary\"]',7,'AvailableCharsets','建库禁用字符集\"%s\"，请使用\"%s\"。','checkboxes/key=CHARsets',4,1549962168,1553774434),
...
('CIX-L2-001','2221dc67-94c6-4a3d-a3b5-638a3d02b70a',15,'组合索引允许的最大列数',2,150,'lte','3',7,'IndexMaxColumnLimit','索引\"%s\"中索引列数量超过允许的阈值%d。','number',1, 0, UNIX_TIMESTAMP()),
...
('CIX-L2-006','47ee8f95-9d48-4a4c-a917-c4679d5fe4f0',15,'组合索引中是否有重复列',2,150,'none','nil',5,'DuplicateIndexColumn','索引\"%s\"中索引了重复的列。','none',1, 0, UNIX_TIMESTAMP()),
...
('CIX-L3-004','2e301212-02af-4b84-955f-3178812ded5a',15,'索引内容是否重复',1,150,'none','nil',4,'IndexOverlay','索引\"%s\"在已有索引\"%s\"相同或者存在覆盖关系。','none',1, 0, UNIX_TIMESTAMP()),
...
('CIX-L3-006','6f69fd99-4d89-4c6f-b8ce-1b9dd437e687',15,'最多能建多少个索引',1,150,'lte','5',6,'IndexCountLimit','索引数量超过允许的阈值%d。','number',1, 0, UNIX_TIMESTAMP()),
...
('CTB-L2-009','6f945479-4788-4bdb-b1e2-e21dc3b335d6',11,'禁止使用CREATE TABLE ... SELECT ...建表',2,110,'none','nil',5,'CreateUseSelectNotAllowed','禁止使用CREATE TABLE AS SELECT的方式建表。','none',1, 0, UNIX_TIMESTAMP()),
...
('CTB-L2-014','2c98b6b7-0e0e-438e-8ccd-fa73fbcdbbea',11,'表允许的最大列数',2,110,'lte','25',7,'ColumnCountLimit','表\"%s\"中定义%d个列，数量超出了规则允许的上限%d，请考虑拆分表。','number',1, 0, UNIX_TIMESTAMP()),
...
('CTB-L2-024','84fcb061-af77-4a7f-8a0e-dfc166b2c531',11,'仅允许一个时间戳类型的列',2,110,'none','nil',7,'TimestampColumnCountLimit','表\"%s\"中的定义了多个时间戳列，请改用DATETIME类型。','none',1, 0, UNIX_TIMESTAMP()),
('CTB-L2-025','0df017c0-d106-4df5-845c-bf50557d497f',11,'单一索引最大列数',2,110,'lte','3',7,'IndexMaxColumnLimit','索引\"%s\"索引的列数超出了规则允许的上限，请控制在%d个列以内。','number',1, 0, UNIX_TIMESTAMP()),
...
('CTB-L2-048','e5c85872-1cff-48c0-b705-0c61b04070b9',11,'表中最多可建多少个索引',2,110,'lte','5',7,'IndexCountLimit','表\"%s\"中定义了%d个索引，数量超过允许的阈值%d。','number',1, 0, UNIX_TIMESTAMP()),
('CTB-L2-049','7eedb2fb-5e26-41a7-baa7-8c37b8ef3170',11,'禁止使用CREATE TABLE ... LIKE ...建表',2,110,'none','nil',5,'UseLikeNotAllowed','禁止使用CREATE TABLE LIKE的方式建表。','none',1, 0, UNIX_TIMESTAMP()),
('CTB-L2-050','5ace374b-0f35-4132-8941-44283bbc87c1',11,'仅允许定义一个自增列',2,110,'nil','nil',5,'AutoIncColumnCountLimit','表\"%s\"中定义了多个自增列。','none',1, 0, UNIX_TIMESTAMP()),
('CTB-L2-051','b9dce6ba-5612-4bc4-afdb-730d7b15930a',11,'仅允许定义一个主键',2,110,'nil','nil',5,'PrimaryKeyCountLimit','表\"%s\"中定义了多个主键。','none',1, 0, UNIX_TIMESTAMP()),
...
('DEL-L3-001','de3855f6-b835-49eb-89ff-bf854c515a02',16,'单次删除的最大行数',1,163,'lte','1000',5,'RowsLimit','单条DELETE语句不得操作超过%d条记录。','number',1, 0, UNIX_TIMESTAMP()),
...
('INS-L2-004','16d0a0b4-3525-449c-965a-673bd290efc7',16,'INSERT时单语句允许操作的最大行数',2,160,'lte','10000',7,'RowsLimit','单条INSERT语句不得操作超过%d条记录。','number',1, 0, UNIX_TIMESTAMP()),
('INS-L2-005','ab221728-8c92-4019-92e4-4cf0fc57941b',16,'INSERT时列类型、值是否匹配',2,160,'none','nil',5,'ColumnValueMatch','INSERT语句的列数量和值数量不匹配。','none',1, 0, UNIX_TIMESTAMP()),
...
('MTB-L2-039','9280ff7c-ff2b-4c3e-bf6c-4886d49634d1',12,'单一索引最大列数',2,120,'lte','3',7,'IndexMaxColumnLimit','索引\"%s\"索引的列数超出了规则允许的上限，请控制在%d个列以内。','number',1, 0, UNIX_TIMESTAMP()),
...
('MTB-L3-006','6677109a-5798-44b9-9f8e-597cce76f168',12,'表允许的最大列数',2,120,'lte','25',6,'ColumnCountLimit','表\"%s\"中定义%d个列，数量超出了规则允许的上限%d，请考虑拆分表。','number',1, 0, UNIX_TIMESTAMP()),
('MTB-L3-007','5e179511-ae0a-4e55-abf6-23120adda97f',12,'仅允许一个时间戳类型的列',2,120,'none','nil',6,'TimestampColumnCountLimit','表\"%s\"中的定义了多个时间戳列，请改用DATETIME类型。','none',1, 0, UNIX_TIMESTAMP()),
...
('MTB-L3-014','08fb5c7f-f4f9-490f-a3a6-a8c4c82bd1a1',12,'覆盖索引检查',1,120,'none','nil',5,'IndexCountLimit','目标表\"%s\"上已存在索引\"%s\"。','none',1, 0, UNIX_TIMESTAMP()),
...
('RPL-L2-004','3901b27f-3fd6-429a-92f3-cbfb327aec6e',16,'REPLACE时单语句允许操作的最大行数',2,161,'lte','1000',7,'RowsLimit','单条REPLACE语句不得操作超过%d条记录。','number',1, 0, UNIX_TIMESTAMP()),
('RPL-L2-005','eaaa918c-611f-467c-9c65-03ff4124a998',16,'REPLACE时列类型、值是否匹配',2,161,'none','nil',5,'ColumnValueMatch','REPLACE语句的列数量和值数量不匹配。','none',1, 0, UNIX_TIMESTAMP()),
...
('SEL-L2-003','b44f75af-51f0-48f5-8d1c-b8f8b1eb225d',17,'禁止SELECT STAR',2,170,'none','nil',7,'StarNotAllowed','禁止SELECT *操作，需要显式指定需要查询的列。','none',1, 0, UNIX_TIMESTAMP()),
('SEL-L2-004','e1018221-684c-49f0-baf4-2ea77c80fd3d',17,'禁止SELECT FOR UPDATE',2,170,'none','nil',5,'ForUpdateNotAllowed','禁止SELECT FOR UPDATE操作。','none',1, 0, UNIX_TIMESTAMP()),
...
('SEL-L3-004','d59e3dad-cf02-41b6-a306-987e1e07814e',17,'是否允许返回BLOB/TEXT列',1,170,'none','nil',4,'BlobOrTextNotAllowed','查询语句中指的列\"%s\"是BLOB/TEXT类型。','none',1, 0, UNIX_TIMESTAMP()),
...
('UPD-L3-005','a72f3d07-7512-46bb-a442-8a505610a72e',16,'允许单次更新的最大行数',1,162,'lte','1000',6,'RowsLimit','单条UPDATE语句不得操作超过%d条记录。','number',1,1549962169,1553774434);
UNLOCK TABLES;
